...
export default class Logil {
    private BASE_CONFIG: LogilConfig = {
        icon: "▬",
        prefix: "Logil:",
        level: 0
    };

    private BASE_PREFIX: PrefixInterface = {
        noPrefix: false
    }
    ...
    private prefix(message: string, icon_color: string, options: PrefixInterface = this.BASE_PREFIX) {
     ...
     * 4: error
     */
    private log(message: string, level: number, options: PrefixInterface = this.BASE_PREFIX) {
        switch (level) {
            case 1:
